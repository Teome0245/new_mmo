...
void PlayerCreationManager::loadLuaStartingItems(Lua* lua) {
	...
	try {
		...
		for (int professionNumber = 0; professionNumber < professions.size(); professionNumber++) {
			...
			Reference<ProfessionDefaultsInfo*> val = professionDefaultsInfo.get(professions.get(professionNumber));
			if (val == nullptr) {
				error() << "Skipping professionSpecificItems for missing profession data: " << professions.get(professionNumber);
