...
#include "client/zone/BotMoveQueue.h"
#include "client/zone/PlayerLocomotion.h"
...
void ClientCoreOptions::resolveDependencies() {
	...
	// Headless bots with waitAfterZone must stay connected (visible in-world).
	int waitAfterZone = get<int>("/waitAfterZone", 0);
	bool stayConnected = get<bool>("/stayConnected", false);
	if (needsLogout && !hasLogoutCharacter && waitAfterZone <= 0 && !stayConnected) {
...
void ClientCore::run() {
	...
	if (waitAfterZone > 0 && zone != nullptr && zone->isSceneReady()) {
		runConnectedLoop(waitAfterZone * 1000);
	}
...
void ClientCore::runConnectedLoop(int waitAfterZoneMs) {
	info(true) << "Staying connected for " << (waitAfterZoneMs / 1000) << " seconds (locomotion + bot_move poll)...";

	std::string botName = options.get<std::string>("/botCharacterName", "");
	if (botName.empty()) {
		botName = options.get<std::string>("/characterFirstname", "Lia");
	}
	std::string queuePath = options.get<std::string>("/botMoveQueue", "ia_bridge/bot_move.jsonl");
	int tickMs = options.get<int>("/botMoveTickMs", 450);

	BotMoveQueue queue(String(queuePath.c_str()), String(botName.c_str()));

	if (locomotion != nullptr && selectedCharacterOid != 0) {
		locomotion->reset(selectedCharacterOid);
	}

	int elapsed = 0;
	while (elapsed < waitAfterZoneMs && zone != nullptr && zone->isSceneReady()) {
		BotMoveCommand cmd = queue.poll();
		if (cmd.valid && locomotion != nullptr) {
			if (cmd.verb == "stop") {
				locomotion->clearGoal();
				info(true) << "bot_move: stop";
			} else if (cmd.verb == "sync_pos") {
				locomotion->syncPosition(cmd.x, cmd.z, cmd.y);
				info(true) << "bot_move: sync_pos " << cmd.x << " " << cmd.y << " " << cmd.z;
			} else if (cmd.verb == "move_to") {
				locomotion->setGoal(cmd.x, cmd.y, cmd.z, cmd.stopRadiusM);
				info(true) << "bot_move: move_to " << cmd.x << " " << cmd.y << " r=" << cmd.stopRadiusM;
			}
		}

		if (locomotion != nullptr && locomotion->isReady()) {
			locomotion->tick(zone);
		}
