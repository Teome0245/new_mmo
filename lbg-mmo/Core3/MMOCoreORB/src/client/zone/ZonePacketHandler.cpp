...
void ZonePacketHandler::handleCmdStartScene(Message* pack) {
	...
	info(true) << "Scene Starting for character OID: " << selfPlayerObjectID << " at " << x << " " << z << " " << y;

	if (core != nullptr && core->getLocomotion() != nullptr) {
		core->getLocomotion()->reset(selfPlayerObjectID);
		core->getLocomotion()->setInitialPosition(x, z, y, 0);
	}
...
void ZonePacketHandler::handleObjectControllerMessage(Message* pack) {
	...
	if (objectID != core->selectedCharacterOid) {
		return;
	}

	// DataTransform serveur → client (correction position)
	if (header2 == 0x71) {
		uint32 timeStamp = pack->parseInt();
		uint32 moveCount = pack->parseInt();
		float dirX = pack->parseFloat();
		float dirY = pack->parseFloat();
		float dirZ = pack->parseFloat();
		float dirW = pack->parseFloat();
		float posX = pack->parseFloat();
		float posZ = pack->parseFloat();
		float posY = pack->parseFloat();
		float speed = pack->parseFloat();

		core->getLocomotion()->applyServerTransform(posX, posZ, posY, dirX, dirY, dirZ, dirW, speed);

		debug() << "DataTransform sync ts=" << timeStamp << " mc=" << moveCount << " pos=" << posX << "," << posY;
	}
}
