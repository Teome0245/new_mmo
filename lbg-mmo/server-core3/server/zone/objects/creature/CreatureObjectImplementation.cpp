...
#include "server/zone/managers/player/LbgAdminAccount.h"
...
void CreatureObjectImplementation::notifyLoadFromDatabase() {
	...
	*/
	...
	unsigned int effectiveAdminLevel = managers::player::getEffectiveAdminLevel(ghost, nullptr);

	auto skillManager = SkillManager::instance();
	const SkillList* playerSkillList = getSkillList();

	int totalSkillPointsWasted = managers::player::getAdminSkillPointBase(effectiveAdminLevel);
	...
	if (ghost->getSkillPoints() != totalSkillPointsWasted) {
		if (effectiveAdminLevel >= managers::player::LBG_ADMIN_LEVEL_ADMIN) {
			ghost->setSkillPoints(totalSkillPointsWasted);
			info(true) << "admin account: skill points set to " << totalSkillPointsWasted
				<< " (base " << managers::player::LBG_ADMIN_SKILL_POINT_BASE << ")";
		} else {
	...
	if (effectiveAdminLevel >= managers::player::LBG_ADMIN_LEVEL_ADMIN) {
		managers::player::applyAdminPermanentAttributes(asCreatureObject());
	}

	// Comptes admin : éjecter uniquement d'une cellule parente invalide (BDD héritée).
	// Ne jamais repositionner le perso dans le monde ouvert (ex. Mos Eisley).
	bool adminStaleParentCleared = false;

	if (effectiveAdminLevel >= 4) {
		Zone* playerZone = getZone();
		uint64 savedParentID = ghost->getSavedParentID();

		if (playerZone != nullptr && savedParentID != 0) {
			ManagedReference<SceneObject*> savedParent = playerZone->getZoneServer()->getObject(savedParentID, true).get();

			if (savedParent == nullptr) {
				if (getParent() != nullptr) {
					playerZone->transferObject(asCreatureObject(), -1, true);
				}

				ghost->setSavedParentID(0);
				adminStaleParentCleared = true;

				info(true) << "admin stale parent cleared on load for account admin_level "
					<< effectiveAdminLevel << " savedParentID=" << savedParentID;
			}
		}
	}
	...
	if (getZone() != nullptr && !adminStaleParentCleared) {
		ghost->setLinkDead();
	}
}
