...
public:
	...
	static void connectApprovedPlayer(SceneObject* obj, uint64_t characterID, CreatureObject* player, ZoneClientSession* client, ZoneServer* zoneServer) {
		...
		*/
		...
		// Comptes admin : éjecter d'une cellule parente invalide uniquement — pas de repositionnement monde.
		unsigned int effectiveAdminLevel = managers::player::getEffectiveAdminLevel(ghost, client);

		if (effectiveAdminLevel >= managers::player::LBG_ADMIN_LEVEL_ADMIN && savedParentID != 0 && playerParent == nullptr) {
			savedParentID = 0;
			...
			player->info(true) << "SelectCharacterCallback admin stale parent cleared for account admin_level "
				<< effectiveAdminLevel;
		}
		...
		if (playerManager != nullptr) {
			...
			if (effectiveAdminLevel >= managers::player::LBG_ADMIN_LEVEL_ADMIN) {
				playerManager->enhanceAdminCharacter(player);
			}
		}
