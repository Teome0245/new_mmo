...
public:
   RequestCategoriesResponseMessage(sys::util::SortedVector<server::zone::managers::holocron::BugCategory>* categories) : BaseMessage() {
		insertShort(0x03);
		...
		for (int i = 0; i < categories->size(); ++i) {
			server::zone::managers::holocron::BugCategory& category = categories->get(i);
