...
void DirectorManager::initializeLuaEngine(Lua* luaEngine) {
	...
	luaEngine->registerFunction("spawnBuildingOnPlateau", spawnBuildingOnPlateau);
	...
	luaEngine->registerFunction("getPlanetHeight", getPlanetHeight);
	luaEngine->registerFunction("addTerrainFlatten", addTerrainFlatten);
	luaEngine->registerFunction("removeTerrainFlatten", removeTerrainFlatten);
	...
	luaEngine->registerFunction("pollIaBridgeCommand", pollIaBridgeCommand);
	luaEngine->registerFunction("writeIaBridgePlayerSnapshot", writeIaBridgePlayerSnapshot);
	luaEngine->registerFunction("iaConfigureGameTime", iaConfigureGameTime);
	luaEngine->registerFunction("iaGetGameTime", iaGetGameTime);
	luaEngine->registerFunction("iaGetDayPhase", iaGetDayPhase);
	luaEngine->registerFunction("iaPublishWorldEvent", iaPublishWorldEvent);
...
int DirectorManager::spawnBuilding(lua_State* L) {
	...
	if (serverTemplate == nullptr) {
		...
		StructureObject* structure = StructureManager::instance()->placeStructure(creature, script, x, y, angle, 1);
...
int DirectorManager::spawnBuildingOnPlateau(lua_State* L) {
	int numberOfArguments = lua_gettop(L);
	if (numberOfArguments != 6) {
		String err = "incorrect number of arguments passed to DirectorManager::spawnBuildingOnPlateau";
		printTraceError(L, err);
		ERROR_CODE = INCORRECT_ARGUMENTS;
		return 0;
	}

	float plateauZ = lua_tonumber(L, -1);
	int angle = lua_tointeger(L, -2);
	float y = lua_tonumber(L, -3);
	float x = lua_tonumber(L, -4);
	String script = lua_tostring(L, -5);
	...
	if (serverTemplate == nullptr) {
		...
		StructureObject* structure = StructureManager::instance()->placeStructure(creature, script, x, y, angle, 1, plateauZ);
		if (structure == nullptr) {
			String err = "Unable to spawn building on plateau " + script;
			printTraceError(L, err);
			lua_pushnil(L);
		} else {
			structure->_setUpdated(true);
...
int DirectorManager::getPlanetHeight(lua_State* L) {
	if (checkArgumentCount(L, 3) == 1) {
		String err = "incorrect number of arguments passed to DirectorManager::getPlanetHeight";
		printTraceError(L, err);
		ERROR_CODE = INCORRECT_ARGUMENTS;
		return 0;
	}
...
int DirectorManager::addTerrainFlatten(lua_State* L) {
	int numberOfArguments = lua_gettop(L);
	if (numberOfArguments != 4 && numberOfArguments != 5) {
		String err = "incorrect number of arguments passed to DirectorManager::addTerrainFlatten";
		printTraceError(L, err);
		ERROR_CODE = INCORRECT_ARGUMENTS;
		return 0;
	}
	...
	String zoneName;
	uint64 modId = 0;
	String layFile = "terrain/poi_small.lay";

	if (numberOfArguments == 5) {
		modId = lua_tointeger(L, -1);
		layFile = lua_tostring(L, -2);
		y = lua_tonumber(L, -3);
		x = lua_tonumber(L, -4);
		zoneName = lua_tostring(L, -5);
	} else {
		y = lua_tonumber(L, -1);
		x = lua_tonumber(L, -2);
		zoneName = lua_tostring(L, -3);
	}

	if (modId == 0)
		modId = 0x4C424700000000ULL ^ (uint64)(int)x ^ ((uint64)(int)y << 16);
	...
	terrainManager->addTerrainModification(x, y, layFile, modId);
	lua_pushinteger(L, modId);
	return 1;
}

int DirectorManager::removeTerrainFlatten(lua_State* L) {
	if (checkArgumentCount(L, 2) == 1) {
		String err = "incorrect number of arguments passed to DirectorManager::removeTerrainFlatten";
		printTraceError(L, err);
		ERROR_CODE = INCORRECT_ARGUMENTS;
		return 0;
	}

	uint64 modId = lua_tointeger(L, -1);
	String zoneName = lua_tostring(L, -2);
	...
	terrainManager->removeTerrainModification(modId);
	return 0;
}
...
int DirectorManager::iaStartDance(lua_State* L) {
	if (checkArgumentCount(L, 2) == 1) {
		String err = "incorrect number of arguments passed to DirectorManager::iaStartDance";
		printTraceError(L, err);
		ERROR_CODE = INCORRECT_ARGUMENTS;
		return 0;
	}
	...
	CreatureObject* creature = (CreatureObject*)lua_touserdata(L, -2);
	String danceName = lua_tostring(L, -1);
	...
	if (performanceIndex == 0 && danceName != "basic") {
		performanceIndex = performanceManager->getPerformanceIndex(PerformanceType::DANCE, "basic", 0);
	}
	if (performanceIndex == 0) {
		// Dernier recours IA : première danse chargée dans le PerformanceManager.
		Performance* fallback = performanceManager->getPerformanceFromIndex(1);
		if (fallback != nullptr && !fallback->isMusic()) {
			performanceIndex = fallback->getPerformanceIndex();
		}
	}
	if (performanceIndex == 0) {
		return 0;
	...
	// Bypass "knows dance" check for IA: démarre la session directement.
...
int DirectorManager::iaFlourish(lua_State* L) {
	if (checkArgumentCount(L, 2) == 1) {
		String err = "incorrect number of arguments passed to DirectorManager::iaFlourish";
		printTraceError(L, err);
		ERROR_CODE = INCORRECT_ARGUMENTS;
		return 0;
	}
	...
	CreatureObject* creature = (CreatureObject*)lua_touserdata(L, -2);
	int flourishNumber = (int)lua_tointeger(L, -1);
	...
	ManagedReference<EntertainingSession*> session = dynamic_cast<EntertainingSession*>(facade.get());
	if (session == nullptr) {
		// Pas en session: rien à flourish.
		return 0;
	}
...
int DirectorManager::iaApplyPilotHeight(lua_State* L) {
	if (checkArgumentCount(L, 2) == 1) {
		String err = "incorrect number of arguments passed to DirectorManager::iaApplyPilotHeight";
		printTraceError(L, err);
		ERROR_CODE = INCORRECT_ARGUMENTS;
		return 0;
	}
	...
	Locker locker(creature);
	creature->setHeight(height, true);
	return 0;
}
...
int DirectorManager::pollIaBridgeCommand(lua_State* L) {
	const char* queuePath = "ia_bridge/pending.jsonl";
	...
	std::ofstream out(queuePath, std::ios::trunc);
	if (out.is_open()) {
	...
	lua_pushlstring(L, first.c_str(), first.size());
	return 1;
}

namespace {
static const char* IA_GT_KEY_START = "ia_bridge:game_time_start_ts";
static const char* IA_GT_KEY_DAY_SEC = "ia_bridge:game_time_day_sec";
static const char* IA_GT_KEY_WORK_SEC = "ia_bridge:game_time_work_sec";
static const char* IA_GT_KEY_REST_SEC = "ia_bridge:game_time_rest_sec";
static const char* IA_GT_KEY_LEISURE_SEC = "ia_bridge:game_time_leisure_sec";

String iaBridgeJsonEscape(const String& value);

static uint64 iaNowSec() {
	return (uint64)time(nullptr);
}

static uint64 iaToU64(const String& s) {
	if (s.isEmpty()) return 0;
	char* end = nullptr;
	const unsigned long long parsed = strtoull(s.toCharArray(), &end, 10);
	if (end == s.toCharArray()) return 0;
	return (uint64)parsed;
}

static String iaFromU64(uint64 v) {
	return String::valueOf(v);
}

static uint64 iaGetOrInitU64(const char* key, uint64 fallback) {
	auto* dm = DirectorManager::instance();
	String cur = dm->getQuestStatus(key);
	uint64 val = iaToU64(cur);
	if (val == 0) {
		val = fallback;
		dm->setQuestStatus(key, iaFromU64(val));
	}
	return val;
}

static void iaSetU64(const char* key, uint64 v) {
	DirectorManager::instance()->setQuestStatus(key, iaFromU64(v));
}
} // namespace

int DirectorManager::iaConfigureGameTime(lua_State* L) {
	// iaConfigureGameTime(daySec, workSec, restSec, leisureSec)
	if (checkArgumentCount(L, 4) == 1) {
		String err = "incorrect number of arguments passed to DirectorManager::iaConfigureGameTime";
		printTraceError(L, err);
		ERROR_CODE = INCORRECT_ARGUMENTS;
		return 0;
	}

	const uint64 daySec = (uint64)lua_tointeger(L, -4);
	const uint64 workSec = (uint64)lua_tointeger(L, -3);
	const uint64 restSec = (uint64)lua_tointeger(L, -2);
	const uint64 leisureSec = (uint64)lua_tointeger(L, -1);

	if (daySec > 0) iaSetU64(IA_GT_KEY_DAY_SEC, daySec);
	if (workSec > 0) iaSetU64(IA_GT_KEY_WORK_SEC, workSec);
	if (restSec > 0) iaSetU64(IA_GT_KEY_REST_SEC, restSec);
	if (leisureSec > 0) iaSetU64(IA_GT_KEY_LEISURE_SEC, leisureSec);

	// start timestamp: if missing, initialize now to avoid 0.
	(void)iaGetOrInitU64(IA_GT_KEY_START, iaNowSec());

	lua_pushboolean(L, 1);
	return 1;
}

int DirectorManager::iaGetGameTime(lua_State* L) {
	// Returns table: { now, start, daySec, inDay, phase="work|rest|leisure" }
	const uint64 now = iaNowSec();
	const uint64 start = iaGetOrInitU64(IA_GT_KEY_START, now);
	const uint64 daySec = iaGetOrInitU64(IA_GT_KEY_DAY_SEC, 6 * 3600);
	const uint64 workSec = iaGetOrInitU64(IA_GT_KEY_WORK_SEC, 2 * 3600);
	const uint64 restSec = iaGetOrInitU64(IA_GT_KEY_REST_SEC, 2 * 3600);
	const uint64 leisureSec = iaGetOrInitU64(IA_GT_KEY_LEISURE_SEC, 2 * 3600);

	const uint64 elapsed = (now >= start) ? (now - start) : 0;
	const uint64 inDay = (daySec > 0) ? (elapsed % daySec) : 0;

	const char* phase = "leisure";
	if (inDay < workSec) {
		phase = "work";
	} else if (inDay < (workSec + restSec)) {
		phase = "rest";
	} else if (inDay < (workSec + restSec + leisureSec)) {
		phase = "leisure";
	}
	...
	lua_settable(L, -3);
	lua_pushstring(L, "daySec");
	lua_pushinteger(L, (lua_Integer)daySec);
	lua_settable(L, -3);
	lua_pushstring(L, "inDay");
	lua_pushinteger(L, (lua_Integer)inDay);
	lua_settable(L, -3);
...
int DirectorManager::iaGetDayPhase(lua_State* L) {
	// Returns string phase (work/rest/leisure)
	const uint64 now = iaNowSec();
	const uint64 start = iaGetOrInitU64(IA_GT_KEY_START, now);
	const uint64 daySec = iaGetOrInitU64(IA_GT_KEY_DAY_SEC, 6 * 3600);
	const uint64 workSec = iaGetOrInitU64(IA_GT_KEY_WORK_SEC, 2 * 3600);
	...
	const char* phase = "leisure";
	if (inDay < workSec) phase = "work";
	else if (inDay < (workSec + restSec)) phase = "rest";
	else if (inDay < (workSec + restSec + leisureSec)) phase = "leisure";

	lua_pushstring(L, phase);
	return 1;
}

int DirectorManager::iaPublishWorldEvent(lua_State* L) {
	// iaPublishWorldEvent(type, payloadJsonOrText) -> bool
	if (checkArgumentCount(L, 2) == 1) {
		String err = "incorrect number of arguments passed to DirectorManager::iaPublishWorldEvent";
		printTraceError(L, err);
		ERROR_CODE = INCORRECT_ARGUMENTS;
		return 0;
	}
	...
	const String payload = lua_tostring(L, -1);
	const uint64 ts = iaNowSec();

	const char* path = "ia_bridge/world_events.jsonl";
	...
	out << "{\"version\":1,\"ts\":" << ts
	    << ",\"type\":\"" << iaBridgeJsonEscape(type).toCharArray()
	    << "\",\"payload\":\"" << iaBridgeJsonEscape(payload).toCharArray()
	    << "\"}\n";
	out.close();
...
int DirectorManager::iaPollWorldEvent(lua_State* L) {
	// iaPollWorldEvent() -> string|nil (one JSON line)
	const char* path = "ia_bridge/world_events.jsonl";
...
bool iaBridgeWriteSnapshotFile(const String& jsonBody) {
	const char* snapshotPath = "ia_bridge/player_snapshot.json";
	const char* tempPath = "ia_bridge/player_snapshot.json.tmp";
	...
	out << jsonBody.toCharArray();
	out.close();

	return std::rename(tempPath, snapshotPath) == 0;
}
} // namespace

int DirectorManager::writeIaBridgePlayerSnapshot(lua_State* L) {
	if (checkArgumentCount(L, 1) == 1) {
		String err = "incorrect number of arguments passed to DirectorManager::writeIaBridgePlayerSnapshot";
		printTraceError(L, err);
		ERROR_CODE = INCORRECT_ARGUMENTS;
		return 0;
	}
	...
	if (player == nullptr) {
		json << "{"
		     << "\"online\":false,"
		     << "\"player\":\"" << iaBridgeJsonEscape(playerName) << "\","
		     << "\"ts\":" << (int64)now
		     << "}";
	} else {
		String zoneName;
		...
		const float posX = player->getWorldPositionX();
		const float posY = player->getWorldPositionY();
		const float posZ = player->getWorldPositionZ();

		json << "{"
		     << "\"online\":true,"
		     << "\"player\":\"" << iaBridgeJsonEscape(player->getFirstName()) << "\","
		     << "\"firstname\":\"" << iaBridgeJsonEscape(player->getFirstName()) << "\","
		     << "\"surname\":\"" << iaBridgeJsonEscape(player->getLastName()) << "\","
		     << "\"zone\":\"" << iaBridgeJsonEscape(zoneName) << "\","
		     << "\"x\":" << posX << ","
		     << "\"y\":" << posY << ","
		     << "\"z\":" << posZ << ","
		     << "\"hp\":" << player->getHAM(0) << ","
		     << "\"action\":" << player->getHAM(3) << ","
		     << "\"mind\":" << player->getHAM(6) << ","
		     << "\"ts\":" << (int64)now
		     << "}";
	}

	if (!iaBridgeWriteSnapshotFile(json.toString())) {
		instance()->error() << "writeIaBridgePlayerSnapshot: impossible d'écrire ia_bridge/player_snapshot.json";
	}
