...
StructureObject* StructureManager::placeStructure(CreatureObject* creature, const String& structureTemplatePath, float x, float y, int angle, int persistenceLevel, float forcedPlateauZ) {
	ManagedReference<Zone*> zone = creature->getZone();
	...
	if (forcedPlateauZ > -15999.f) {
		z = forcedPlateauZ;
	} else if (floraRadius > 0 && !snapToTerrain) {
		float hi = terrainManager->getHighestHeight(x + w0, y + l0, x + w1, y + l1, 1);
		float lo = terrainManager->getLowestHeight(x + w0, y + l0, x + w1, y + l1, 1);
		float slope = hi - lo;
		// Pentes douces : Z moyen (alignement serveur/client sur plateaux type Lost Heaven).
		if (slope <= 6.f)
			z = (hi + lo) * 0.5f + zIncreaseWhenNoAvailableFootprint;
		else
			z = hi + zIncreaseWhenNoAvailableFootprint;
	}
