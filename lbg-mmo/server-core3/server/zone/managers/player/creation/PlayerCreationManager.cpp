...
#include "server/login/account/AdminLevelCompat.h"
...
void PlayerCreationManager::loadLuaConfig() {
	...
	inheritAccountAdminLevel = lua->getGlobalByte("inheritAccountAdminLevel") != 0;
	...
	if (ghost != nullptr) {
		...
		if (!freeGodMode) {
			try {
				...
				int accountPermissionLevel = normalizeAdminLevelFromLegacy(playerAccount->getAdminLevel());
				String accountName = playerAccount->getUsername();

				// LBG ADR 0006 : seul Admin (4) peut hériter le staff sur un nouveau perso.
				if (inheritAccountAdminLevel && accountPermissionLevel == 4) {
					playerManager->updatePermissionLevel(playerCreature, accountPermissionLevel);
				}

				// Délai entre créations (comptes < Admin 4). 0 = désactivé (lab LAN).
				// Défaut SWGEmu public : 3600000 ms (1 h) — anti-spam création/suppression.
				int createCooldownMs = ConfigManager::instance()->getInt(
					"Core3.PlayerCreationManager.CharacterCreateCooldownMs", 3600000);

				if (accountPermissionLevel < 4 && createCooldownMs > 0) {
					...
					try {
						...
						if (res != nullptr && res->next()) {
							...
							if (timeVal.miliDifference() < createCooldownMs) {
				...
				// No separate check needed
					...
					if (lastCreatedCharacter.containsKey(accID)) {
						...
						if (lastCreatedTime.miliDifference() < createCooldownMs) {
							ErrorMessage* errMsg = new ErrorMessage("Create Error", "You are only permitted to create one character per hour. Repeat attempts prior to 1 hour elapsing will reset the timer.", 0x0);
							client->sendMessage(errMsg);
