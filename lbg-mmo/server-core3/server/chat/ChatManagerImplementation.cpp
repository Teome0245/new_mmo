...
namespace {
String lbgIaJsonEscape(const String& value) {
	std::string input(value.toCharArray());
	std::string output;
...
String lbgIaSpatialChatTarget(const String& actor, const String& message) {
	String actorLower = actor.toLowerCase();

	if (actorLower == "lia" || actorLower == "nix")
		return "";

	String lower = message.toLowerCase();

	if (lower.indexOf("lia") >= 0)
		return "Lia";

	if (lower.indexOf("nix") >= 0)
		return "Nix";

	return "";
}

void lbgIaAppendSpatialChatEvent(CreatureObject* player, const UnicodeString& message) {
	if (player == nullptr || !player->isPlayerCreature())
		return;

	String actor = player->getFirstName();
	String msg = message.toString();
	String target = lbgIaSpatialChatTarget(actor, msg);

	if (target.isEmpty())
		return;

	std::ofstream out("ia_bridge/events.jsonl", std::ios::app);
	...
	out << "{\"version\":1"
		<< ",\"event_id\":\"chat-" << now << "-" << player->getObjectID() << "\""
		<< ",\"ts\":" << now
		<< ",\"type\":\"core3.player_spatial_chat\""
		<< ",\"actor\":\"" << lbgIaJsonEscape(actor).toCharArray() << "\""
		<< ",\"target\":\"" << lbgIaJsonEscape(target).toCharArray() << "\""
		<< ",\"message\":\"" << lbgIaJsonEscape(msg).toCharArray() << "\""
		<< ",\"zone\":\"" << (player->getZone() != nullptr ? player->getZone()->getZoneName().toCharArray() : "") << "\""
		<< ",\"x\":" << player->getPositionX()
		<< ",\"y\":" << player->getPositionY()
		<< ",\"z\":" << player->getPositionZ()
		<< ",\"source_line\":\"" << lbgIaJsonEscape(msg).toCharArray() << "\""
		<< "}\n";
}
}
...
void ChatManagerImplementation::handleSpatialChatInternalMessage(CreatureObject* player, const UnicodeString& args) {
	...
	try {
		...
		lbgIaAppendSpatialChatEvent(player, formattedMessage);
