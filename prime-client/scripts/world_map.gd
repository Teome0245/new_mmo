...
func _apply_scale() -> void:
	...
	if tex_size.x <= 0.0:
		return
	var world_span: float = 2.0 * half_size * Projection3D2D.SCALE
	var s: float = world_span / tex_size.x
	var sy: float = -s if flip_texture_y else s
	_sprite.scale = Vector2(s, sy)
